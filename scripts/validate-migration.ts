...
function validateImplementation() {
  ...
  validate(
    "1. Monorepo Layout",
    "Create authoritative monorepo layout",
    checkFileExists("apps/ui") &&
      checkFileExists("apps/server") &&
      checkFileExists("packages/core")
      ? "PASS"
      : "FAIL",
  ...
  validate(
    "1. Monorepo Layout",
    "Retire legacy FastAPI/Python stack",
  ...
  validate(
    "1. Monorepo Layout",
    "Remove control-plane/admin theme and RBAC logic",
  ...
  validate(
    "2. Data Model",
    "Build SQLite schema",
    checkFileExists("packages/db/schema.ts") &&
      checkFileExists("packages/db/migrations/0000_initial.sql")
      ? "PASS"
      : "FAIL",
  ...
  validate(
    "2. Data Model",
    "Persona importer uses catalog defaults",
  ...
  validate(
    ...
    checkFileExists("packages/providers/groq.ts") &&
      ...
      checkFileExists("packages/providers/lmstudio.ts")
      ? "PASS"
      : "FAIL",
  ...
  validate(
    "3. Provider Adapters",
    "Unified provider API with model caching",
  ...
  validate(
    "4. Routing",
    "Routing matrix and resolution algorithm",
  ...
  validate(
    "5. Eyes Orchestrator",
    "Tools registry and runEye logic",
    checkFileExists("packages/core/registry.ts") &&
      checkFileExists("packages/core/orchestrator.ts")
      ? "PASS"
      : "FAIL",
  ...
  validate(
    "6. Bun Server",
    "Implement Hono server with routes",
    checkFileExists("apps/server/src/index.ts") &&
      checkFileExists("apps/server/src/start.ts")
      ? "PASS"
      : "FAIL",
  ...
  validate(
    "7. UI",
    "Convert to Next.js 15 with Overseer theme",
    checkFileExists("apps/ui/src/app/layout.tsx") &&
      checkFileExists("apps/ui/src/app/page.tsx")
      ? "PARTIAL"
      : "FAIL",
  ...
  validate(
    "8. CLI",
    "NPX entrypoint implementation",
    checkFileExists("cli/index.ts") && checkFileExists("package.json")
      ? "PASS"
      : "FAIL",
  ...
  console.log(`\n📈 Summary:`);
  console.log(
    `  ✅ Complete: ${passCount}/${total} (${Math.round((passCount / total) * 100)}%)`,
  );
  console.log(
    `  ⚠️  Partial: ${partialCount}/${total} (${Math.round((partialCount / total) * 100)}%)`,
  );
  console.log(
    `  ❌ Missing: ${failCount}/${total} (${Math.round((failCount / total) * 100)}%)`,
  );

  const completionPercentage = Math.round(
    ((passCount + partialCount * 0.5) / total) * 100,
  );
