...
function validateOpenRouterKey(key: string): boolean | string {
  if (!key) return true; // Allow empty (skip)
  if (!key.startsWith('sk-or-')) return 'OpenRouter API keys must start with "sk-or-"';
...
export async function firstTimeSetup(): Promise<SetupConfig> {
  ...
  console.log(kleur.white('Let\'s configure your AI providers to get started.\n'));
...
export function writeEnvConfig(config: SetupConfig): void {
  ...
  if (config.providers.groq) {
    envContent = envContent.replace(/^GROQ_API_KEY=.*$/m, `GROQ_API_KEY=${config.providers.groq}`);
  }

  if (config.providers.openrouter) {
    envContent = envContent.replace(/^OPENROUTER_API_KEY=.*$/m, `OPENROUTER_API_KEY=${config.providers.openrouter}`);
  }

  if (config.encryption_key) {
    envContent = envContent.replace(/^THIRD_EYE_SECURITY_ENCRYPTION_KEY=.*$/m, `THIRD_EYE_SECURITY_ENCRYPTION_KEY=${config.encryption_key}`);
  }
  ...
  if (config.providers.lmstudio) console.log(kleur.green('  ✓ LM Studio (local)'));
