...
const SYSTEM_PIPELINE = {
  ...
  version: 1,
  description:
    "Comprehensive multi-Eye validation workflow demonstrating all pipeline capabilities including sequential validation, conditional branching, and user approval gates.",
  category: "built-in",
  workflow: {
    steps: [
      {
      ...
      },
      {
        ...
        eye: "kyuubi",
        description:
          "Restructure prompt into ROLE/TASK/CONTEXT/REQUIREMENTS/OUTPUT format",
        next: "confirm_intent",
      },
      {
        id: "confirm_intent",
      ...
      },
      {
        ...
        prompt:
          "Please submit your implementation plan following the provided schema",
        next: "plan_review",
      },
      {
        id: "plan_review",
...
async function seedSystemPipeline() {
  ...
  try {
    ...
    if (existing.length > 0) {
      console.log(
        "⚠️  System pipeline already exists. Deactivating old versions...",
      );
    ...
    console.log(
      `   Eyes used: ${SYSTEM_PIPELINE.workflow.steps
        .filter((s) => s.eye)
        .map((s) => s.eye)
        .join(", ")}`,
    );
    console.log(
      `   Conditional branches: ${SYSTEM_PIPELINE.workflow.steps.filter((s) => s.type === "condition").length}`,
    );
    console.log(
      `   User input steps: ${SYSTEM_PIPELINE.workflow.steps.filter((s) => s.type === "user_input").length}`,
    );
