...
async function runMigrations() {
  ...
  try {
    ...
    log(
      "  This is normal on first run. Database will be created automatically.",
      "cyan",
    );
  }
}
...
async function seedDatabase() {
  ...
  try {
    ...
    log(
      "  You can manually seed later with: bun run scripts/seed-defaults.ts",
      "yellow",
    );
  }
}
...
function printNextSteps(overseerDir: string) {
  ...
  log(
    `            "args": ["run", "${resolve(process.cwd(), "bin/mcp-server.ts")}"]`,
    "reset",
  );
  log("          }", "reset");
  log("        }", "reset");
...
async function main() {
  ...
  try {
    ...
    if (!isValid) {
      log(
        "\n⚠ Some validation checks failed. The setup may be incomplete.",
        "yellow",
      );
      log(
