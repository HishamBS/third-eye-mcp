...
Status : ${response.status} ${response.statusText}`);
    ...
    if (body.checks) {
      ...
      if (body.checks.database) {
        console.log(
          `  • database : ${body.checks.database.ok ? "ok" : "fail"} (${body.checks.database.latency_ms ?? "n/a"} ms)`,
        );
      }
      if (body.checks.providers) {
        for (const [provider, value] of Object.entries(
          body.checks.providers as Record<string, boolean>,
        )) {
          console.log(
            `  • provider ${provider.padEnd(10)}: ${value ? "ok" : "fail"}`,
          );
        }
      }
    }
    ...
    console.error(
      "\n❌ Failed to fetch health endpoint:",
      error instanceof Error ? error.message : error,
    );
    console.error(
      "   Ensure the server is running (bun run dev) before executing this script.",
    );
    process.exit(1);
  }
}
