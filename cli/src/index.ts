...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      if (!isDev) {
        spinner.fail(
          "Server not found. Make sure you're in the project directory.",
        );
        process.exit(1);
      }
...
db.command("reset")
  ...
  .action(async (options) => {
    if (!options.yes) {
      console.log(
        chalk.yellow("⚠️  This will delete ALL data in the database."),
      );
    ...
    try {
      ...
      const { sessions, runs, personas, eyesRouting, providerKeys } =
        await import("@third-eye/db");
