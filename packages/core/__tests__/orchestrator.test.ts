  ...
  beforeAll(async () => {
    ...
    if (existingPersona.length === 0) {
      await db.insert(personas).values({
        ...
        version: 1,
        content:
          "You are a helpful code generation assistant. Respond ONLY with valid JSON envelope.",
        active: true,
        createdAt: new Date(),
      });
    }
  ...
  test("should run Eye with valid configuration (integration)", async () => {
    ...
    try {
      ...
      if (!envelope.ok) {
        ...
        expect(envelope.code).toMatch(/E_/);
        console.log(
          "⏭️  Skipping full integration test (provider not available)",
        );
      } else {
      ...
      console.log(
        "⏭️  Skipping integration test (provider error):",
