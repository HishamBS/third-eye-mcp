...
describe("AutoRouter - Dynamic Routing Integration", () => {
  ...
  describe("Vague Request → Overseer includes Sharingan", () => {
    it("should route through sharingan for vague new task", async () => {
      ...
      // Mock Eye responses
      vi.spyOn(mockOrchestrator, "createSession").mockResolvedValue({
        sessionId: "test-session-1",
      });
      vi.spyOn(mockOrchestrator, "runEye").mockImplementation(
        async (eye: string) => {
      ...
      const eyeCalls = (
        mockOrchestrator.runEye as unknown as Mock
      ).mock.calls.map((call) => call[0] as string);
      expect(eyeCalls).toEqual([
        "overseer",
        "sharingan",
        "kyuubi",
        "jogan",
  ...
  describe("Clear Request → Overseer skips Sharingan", () => {
    it("should skip sharingan for clear new task", async () => {
      const task =
        "Generate 500-word indoor palm care guide for Saudi beginners with watering schedule";
      ...
      const overseerResponse: BaseEnvelope = {
        ...
        data: {
          ...
          routingReasoning:
            "Clear and specific request, only needs intent confirmation and validation",
          skipReasons: {
    ...
    it("should skip guidance eyes for review content", async () => {
      ...
      const overseerResponse: BaseEnvelope = {
        ...
        data: {
          ...
          routingReasoning:
            "Code review with content provided, route directly to validation",
          skipReasons: {
  ...
  describe("Error Handling", () => {
    it("should handle invalid Overseer response gracefully", async () => {
      ...
      vi.spyOn(mockOrchestrator, "runEye").mockResolvedValue(
        invalidOverseerResponse as any,
      );
    ...
    it("should route through multiple validators for mixed content", async () => {
      ...
      const overseerResponse: BaseEnvelope = {
        ...
        data: {
          ...
          routingReasoning:
            "Mixed content requires code and documentation validation",
          skipReasons: {
