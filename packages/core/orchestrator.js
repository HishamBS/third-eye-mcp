...
import { runs, sessions, eyesRouting, providerKeys, providerFailovers } from '@third-eye/db';
import { ProviderFactory } from '@third-eye/providers';
import { getEye, getAllEyeNames } from '@third-eye/eyes';
...
import { RETRY_CONFIG, FALLBACK_CONFIG, categorizeRetryReason } from '@third-eye/constants';
...
function hasBaseUrl(value) {
    return typeof value === 'object' && value !== null && 'baseUrl' in value;
}
const VALID_EYE_NAMES = new Set(getAllEyeNames());
function isEyeName(value) {
    return VALID_EYE_NAMES.has(value);
}
function isRejectedResponse(response) {
...
export class EyeOrchestrator {
     ...
     */
    async runEye(eyeName, input, sessionId, options = {}) {
        ...
        try {
            ...
            try {
                ...
                if (wsManager && 'emitEyeStarted' in wsManager && typeof wsManager.emitEyeStarted === 'function') {
                    const eyeIcon = this.getEyeIcon(eyeName);
                    wsManager.emitEyeStarted(actualSessionId, eyeName, {
            ...
            if (!blueprint) {
                return this.createErrorEnvelope(eyeName, `No blueprint found for Eye: ${eyeName}`, runId, actualSessionId, startTime);
            }
            // Variables to hold successful results
            let envelope = null;
            let completion = null;
            let latencyMs = 0;
            let successfulProviderType = null;
            ...
            for (const providerConfig of providerChain) {
                ...
                try {
                    ...
                    while (attempt < MAX_PERSONA_RETRIES) {
                        ...
                        try {
                            completion = (await retryWithThrow(async () => provider.complete({
                                model: targetModel,
                                messages: [
                            ...
                            if (envelope && 'next' in envelope && envelope.next === undefined && 'next_action' in envelope && envelope.next_action) {
                                envelope.next = envelope.next_action;
                            }
                            ...
                            if (!guardResult.valid) {
                                ...
                                if (attempt < MAX_PERSONA_RETRIES) {
                                    ...
                                    throw new Error(`Persona contract violated after ${MAX_PERSONA_RETRIES} attempts: ${guardResult.violations.map((v) => v.message).join('; ')}`);
                                }
                            }
            ...
            if (!envelope || !successfulProviderType || !successfulModel || !completion) {
                return this.createErrorEnvelope(eyeName, `Failed to get valid response from any provider in chain. Last error: ${lastError}`, runId, actualSessionId, startTime);
            }
