import { CLARIFICATION_FIELD_PROMPTS, REQUIRED_CLARIFICATION_FIELDS, } from '@third-eye/constants';
const SHARINGAN_ID = 'sharingan';
const SHARINGAN_GUIDANCE_CODES = new Set([
    'NEED_CLARIFICATION',
    'NEED_MORE_CONTEXT',
    'REJECT_AMBIGUOUS',
]);
const SHARINGAN_APPROVAL_CODES = new Set(['OK_NO_CLARIFICATION_NEEDED']);
const isObject = (value) => typeof value === 'object' && value !== null;
const normaliseQuestion = (entry) => {
...
const assertSharinganQuestions = (envelope) => {
    const data = (envelope.data ?? {});
    const rawQuestions = Array.isArray(data.questions) ? data.questions : [];
    if (rawQuestions.length !== REQUIRED_CLARIFICATION_FIELDS.length) {
        throw new EyeBehaviorError(SHARINGAN_ID, `questions_missing_canonical_fields_${rawQuestions.length}`);
    }
    REQUIRED_CLARIFICATION_FIELDS.forEach((expectedField, index) => {
        ...
        if (id !== null && id !== expectedField) {
            throw new EyeBehaviorError(SHARINGAN_ID, `questions_canonical_id_mismatch_${id}_${expectedField}`);
        }
        if (text !== canonicalPrompt) {
            throw new EyeBehaviorError(SHARINGAN_ID, `questions_canonical_prompt_mismatch_${expectedField}`);
        }
    });
};
const assertSharinganResolution = (envelope) => {
    const data = (envelope.data ?? {});
    const resolved = isObject(data.resolved) ? data.resolved : null;
    if (!resolved) {
...
const ensureSharinganBehavior = (envelope) => {
    ...
    if (envelope.ok) {
        if (!SHARINGAN_APPROVAL_CODES.has(envelope.code)) {
            throw new EyeBehaviorError(SHARINGAN_ID, `approval_status_not_allowed_${envelope.code ?? 'unknown'}`);
        }
        const data = (envelope.data ?? {});
    ...
    if (!SHARINGAN_GUIDANCE_CODES.has(envelope.code)) {
        throw new EyeBehaviorError(SHARINGAN_ID, `clarification_status_not_allowed_${envelope.code ?? 'unknown'}`);
    }
...
export class EyeBehaviorError extends Error {
    eyeId;
    reason;
    constructor(eyeId, reason) {
        super(`Eye "${eyeId}" violated persona contract: ${reason}`);
        this.eyeId = eyeId;
        this.reason = reason;
    }
}
export const ensureEyeBehavior = (eyeId, envelope) => {
    switch (eyeId) {
        case SHARINGAN_ID:
