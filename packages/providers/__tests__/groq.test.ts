...
describe("GroqProvider", () => {
  ...
  describe("JSON Mode Enforcement", () => {
        ...
        messages: [
          {
            role: "system",
            content:
              'You must respond with valid JSON only. Return an object with "message" and "timestamp" fields.',
          },
        ...
        messages: [
          {
            role: "system",
            content:
              "Return a JSON object with: ok (boolean), code (string), md (string), data (object), next (string)",
          },
    ...
    test("should pass response_format to API correctly", async () => {
      if (!process.env.GROQ_API_KEY) {
        console.log(
          "⏭️  Skipping Groq response_format passthrough test (no API key)",
        );
        return;
      }
