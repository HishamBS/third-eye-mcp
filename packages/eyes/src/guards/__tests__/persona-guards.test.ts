...
import { ensureEyeBehavior, buildReminderMessage, validateStatusCode } from '../persona-guards';
...
describe('Persona Guards', () => {
  describe('ensureEyeBehavior', () => {
    ...
    it('should fail when tag is missing', () => {
      ...
      const result = ensureEyeBehavior(OVERSEER_BLUEPRINT, envelope);
      expect(result.valid).toBe(false);
      const tagViolation = result.violations.find(v => v.field === EnvelopeField.TAG);
    ...
    it('should fail when tag is wrong', () => {
      ...
      const result = ensureEyeBehavior(OVERSEER_BLUEPRINT, envelope);
      expect(result.valid).toBe(false);
      const violation = result.violations.find(v => v.field === EnvelopeField.TAG);
    ...
    it('should fail when required data keys are missing', () => {
      ...
      const result = ensureEyeBehavior(OVERSEER_BLUEPRINT, envelope);
      expect(result.valid).toBe(false);
      const dataViolations = result.violations.filter(v => v.field.startsWith(EnvelopeField.DATA));
    ...
    it('should fail when required UI keys are missing', () => {
      const envelope = {
        ...
        [EnvelopeField.OK]: true,
        [EnvelopeField.CODE]: EyeStatusCode.OK,
        [EnvelopeField.DATA]: { requestType: 'new_task', contentDomain: 'mixed' },
        [EnvelopeField.UI]: {},
      };

      const result = ensureEyeBehavior(OVERSEER_BLUEPRINT, envelope);
      expect(result.valid).toBe(false);
      const uiViolations = result.violations.filter(v => v.field.startsWith(EnvelopeField.UI));
    ...
    it('should detect when ok is not boolean', () => {
      ...
      const result = ensureEyeBehavior(OVERSEER_BLUEPRINT, envelope);
      expect(result.valid).toBe(false);
      const okViolation = result.violations.find(v => v.field === EnvelopeField.OK);
