...
describe("Sharingan Eye", () => {
  describe("Ambiguity Detection", () => {
    it("should detect high ambiguity in vague prompts", () => {
      ...
      expect(response.data[DataKey.QUESTIONS_MD]).toContain(
    ...
    it("should detect low ambiguity in specific prompts", () => {
      const request: SharinganRequest = {
        payload: {
          prompt:
            "Create a TypeScript function named calculateSum that takes two numbers as parameters and returns their sum. Include JSDoc comments with parameter descriptions and return type.",
        },
      };
    ...
    it("should calculate higher scores for shorter prompts", () => {
      ...
      const longRequest: SharinganRequest = {
        payload: {
          prompt:
            "Fix the authentication bug in the login component where users with special characters in their email addresses cannot log in successfully due to incorrect email validation regex",
        },
      };
      ...
      expect(shortResponse.data[DataKey.SCORE]).toBeGreaterThan(
        longResponse.data[DataKey.SCORE],
      );
    });
  });
  ...
  describe("Code Detection", () => {
    it("should detect code-related prompts with framework keywords", () => {
      const request: SharinganRequest = {
        payload: {
          prompt:
            "Write a React component for user authentication with TypeScript",
        },
      };
      ...
      const reasoning = (
        response.data[DataKey.REASONING_MD] as string
      ).toLowerCase();
    ...
    it("should detect code-related prompts with file extensions", () => {
      const request: SharinganRequest = {
        payload: {
          prompt: "Create a new file called utils.ts with helper functions",
        },
      };
    ...
    it("should detect non-code prompts", () => {
      ...
      expect(response.data[DataKey.REASONING_MD]).toContain(
        "No explicit code indicators",
      );
    });
  ...
  describe("Reasoning", () => {
    ...
    it("should provide clear reasoning for low ambiguity score", () => {
      const request: SharinganRequest = {
        payload: {
          prompt:
            "Create a new TypeScript class named UserRepository with methods for CRUD operations on user entities, including proper error handling and type safety",
        },
      };
  ...
  describe("Response Structure", () => {
    ...
    it("should set correct next_action for code-related prompts", () => {
      const request: SharinganRequest = {
        payload: {
          prompt:
            "Create a well-documented TypeScript interface for a User model with id, name, email, and createdAt fields",
        },
      };
      ...
      if (
        !response.data[DataKey.AMBIGUOUS] &&
        response.data[DataKey.IS_CODE_RELATED]
      ) {
