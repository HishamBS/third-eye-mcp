...
export const PromptSection = freezeTokens({
  ...
  RESPONSE_FORMAT_INSTRUCTION: 'You MUST respond with ONLY a JSON object. No Markdown, no explanations, no code blocks.',
  ...
  EXAMPLE_OUTRO: 'Copy the structure, but use values appropriate for your current task.',
  CURRENT_CONTEXT: '## Current Context',
  YOUR_TASK: '## Your Task',
  TASK_OUTRO: 'Analyze the context above and generate a response following the schema and example provided.',
...
export const SelfCheckItem = freezeTokens({
  REQUIRED_FIELDS: '- All required fields are present (tag, ok, code, data, ui, next)',
  STATUS_CODE: '- Status code is from the allowed codes list',
  DATA_STRUCTURE: '- Data structure matches the skeleton',
  UI_FIELDS: '- UI fields (title, summary, details, icon, color) are all present',
  NEXT_ACTION: '- Next action is appropriate for the response',
  NO_STRING_LITERALS: '- No string literals are hardcoded - use JSON schema values',
} as const);
