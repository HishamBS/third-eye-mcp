...
export const EYE_CAPABILITY_LABELS: Record<EyeCapability, string> = Object.freeze({
...
export const CAPABILITY_CATEGORY_MAP: Record<EyeCapability, CapabilityCategory> = Object.freeze({
...
export const CAPABILITY_CATEGORY_LABELS: Record<CapabilityCategory, string> = Object.freeze({
...
> = Object.freeze({
  ...
  [EyeId.KYUUBI]: [EyeCapability.PROMPT_STRUCTURING, EyeCapability.REQUIREMENTS_SYNTHESIS],
  [EyeId.JOGAN]: [
...
const toReadonlyArray = <T>(items: readonly T[]): readonly T[] => Object.freeze(items.slice());
...
const createMembershipChecker = <T extends string>(group: readonly T[]): ((code: string) => code is T) => {
  const membership = new Set(group);
  return (code: string): code is T => membership.has(code as T);
};

export const isApprovedEyeStatusCode = createMembershipChecker(EyeStatusCodeGroup.APPROVED);
export const isNeedsInputEyeStatusCode = createMembershipChecker(EyeStatusCodeGroup.NEEDS_INPUT);
export const isRevisionEyeStatusCode = createMembershipChecker(EyeStatusCodeGroup.REVISION);
export const isErrorEyeStatusCode = createMembershipChecker(EyeStatusCodeGroup.ERROR);
