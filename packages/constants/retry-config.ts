 ...
 */
export const NON_RETRYABLE_STATUS_CODES = Object.freeze(new Set([
  400, // Bad Request
  401, // Unauthorized
  403, // Forbidden
  404, // Not Found
 ...
 */
export const RETRYABLE_ERROR_TYPES = Object.freeze(new Set([
...
export function calculateBackoffDelay(attempt: number): number {
  const exponentialDelay = RETRY_CONFIG.BASE_DELAY_MS * Math.pow(RETRY_CONFIG.BACKOFF_MULTIPLIER, attempt);
...
export function categorizeRetryReason(error: unknown): string {
  ...
  if (typeof error === 'object' && error !== null) {
    ...
    // Timeout
    if (err.status === 408 || err.statusCode === 408 || err.code === 'ETIMEDOUT') {
