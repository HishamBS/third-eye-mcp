...
describe("Clarifications Constants", () => {
  ...
  describe("CLARIFICATION_FIELDS", () => {
    it("should contain all canonical fields", () => {
      ...
      expect(CLARIFICATION_FIELDS).toContain(
  ...
  describe("CLARIFICATION_FIELD_SET", () => {
    it("should be a Set containing all canonical fields", () => {
      ...
      expect(
        CLARIFICATION_FIELD_SET.has(ClarificationFieldTokens.AUDIENCE),
      ).toBe(true);
      expect(
        CLARIFICATION_FIELD_SET.has(ClarificationFieldTokens.DELIVERABLE),
      ).toBe(true);
      expect(CLARIFICATION_FIELD_SET.has(ClarificationFieldTokens.SCOPE)).toBe(
        true,
      );
      expect(
        CLARIFICATION_FIELD_SET.has(ClarificationFieldTokens.SUCCESS_CRITERIA),
      ).toBe(true);
      expect(
        CLARIFICATION_FIELD_SET.has(ClarificationFieldTokens.REFERENCES),
      ).toBe(true);
    });
  });
  ...
  describe("isClarificationField", () => {
    it("should return true for valid clarification fields", () => {
      expect(isClarificationField(ClarificationFieldTokens.AUDIENCE)).toBe(
        true,
      );
      expect(isClarificationField(ClarificationFieldTokens.DELIVERABLE)).toBe(
        true,
      );
      expect(isClarificationField(ClarificationFieldTokens.SCOPE)).toBe(true);
      expect(
        isClarificationField(ClarificationFieldTokens.SUCCESS_CRITERIA),
      ).toBe(true);
      expect(isClarificationField(ClarificationFieldTokens.REFERENCES)).toBe(
        true,
      );
    });
  ...
  describe("isClarificationFieldToken", () => {
    it("should work as an alias for isClarificationField", () => {
      expect(isClarificationFieldToken(ClarificationFieldTokens.AUDIENCE)).toBe(
        true,
      );
      expect(
        isClarificationFieldToken(ClarificationFieldTokens.DELIVERABLE),
      ).toBe(true);
  ...
  describe("CLARIFICATION_FIELD_PROMPTS", () => {
    it("should have prompts for all canonical fields", () => {
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.AUDIENCE],
      ).toBeDefined();
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.DELIVERABLE],
      ).toBeDefined();
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.SCOPE],
      ).toBeDefined();
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.SUCCESS_CRITERIA],
      ).toBeDefined();
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.REFERENCES],
      ).toBeDefined();
    });
    ...
    it("should have canonical question text", () => {
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.AUDIENCE],
      ).toContain("Who is the audience");
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.DELIVERABLE],
      ).toContain("What concrete deliverable");
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.SCOPE],
      ).toContain("constraints, scope boundaries");
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.SUCCESS_CRITERIA],
      ).toContain("success criteria");
      expect(
        CLARIFICATION_FIELD_PROMPTS[ClarificationFieldTokens.REFERENCES],
      ).toContain("references, logs");
    });
  ...
  describe("REQUIRED_CLARIFICATION_FIELDS", () => {
    it("should match CLARIFICATION_FIELDS", () => {
      expect(REQUIRED_CLARIFICATION_FIELDS.length).toBe(
        CLARIFICATION_FIELDS.length,
      );
  ...
  describe("CLARIFICATION_FALLBACKS", () => {
    it("should have fallbacks for all canonical fields", () => {
      expect(CLARIFICATION_FALLBACKS[ClarificationFieldTokens.AUDIENCE]).toBe(
        "Audience not specified",
      );
      expect(
        CLARIFICATION_FALLBACKS[ClarificationFieldTokens.DELIVERABLE],
      ).toBe("Deliverable not specified");
      expect(CLARIFICATION_FALLBACKS[ClarificationFieldTokens.SCOPE]).toBe(
        "Scope requires clarification",
      );
      expect(
        CLARIFICATION_FALLBACKS[ClarificationFieldTokens.SUCCESS_CRITERIA],
      ).toBe("Success criteria not specified");
      expect(CLARIFICATION_FALLBACKS[ClarificationFieldTokens.REFERENCES]).toBe(
