...
describe("Capability Plan Resolver", () => {
  ...
  describe("CapabilityPlan structure", () => {
    ...
    it("should have stage for each assignment", () => {
      ...
      for (const assignment of plan.assignments) {
        ...
        expect([EyeStageToken.GUIDANCE, EyeStageToken.VALIDATION]).toContain(
          assignment.stage,
        );
      }
    });
  ...
  describe("Route ordering", () => {
    it("should have guidance stages before validation stages", () => {
      ...
      const guidanceCount = plan.assignments.filter(
        (a) => a.stage === EyeStageToken.GUIDANCE,
      ).length;
      const validationCount = plan.assignments.filter(
        (a) => a.stage === EyeStageToken.VALIDATION,
      ).length;
      ...
      try {
        resolveCapabilityPlan(RequestType.NEW_TASK, ContentDomain.MIXED, {
          availableEyes: [
            {
              id: "incomplete-eye",
              capabilities: ["random_capability"] as const,
            },
          ],
        });
  ...
  describe("Different content domains produce different routes", () => {
    it("should produce different routes for CODE vs TEXT domains", () => {
      try {
        const codePlan = resolveCapabilityPlan(
          RequestType.NEW_TASK,
          ContentDomain.CODE,
        );
        const textPlan = resolveCapabilityPlan(
          RequestType.NEW_TASK,
          ContentDomain.TEXT,
        );
    ...
    it("should produce different routes for NEW_TASK vs VALIDATION_ONLY", () => {
      const newTaskPlan = resolveCapabilityPlan(
        RequestType.NEW_TASK,
        ContentDomain.MIXED,
      );
      const validationPlan = resolveCapabilityPlan(
        RequestType.VALIDATION_ONLY,
        ContentDomain.MIXED,
      );

      expect(newTaskPlan.route.length).toBeGreaterThan(
        validationPlan.route.length,
      );
    });
  });
  ...
  describe("Assignment completeness", () => {
    ...
    it("should have same length for capabilities and capabilityLabels", () => {
      ...
      for (const assignment of plan.assignments) {
        expect(assignment.capabilities.length).toBe(
          assignment.capabilityLabels.length,
        );
      }
    });
  });
