...
export function exportSessionAsMarkdown(
  ...
  events.forEach((event, idx) => {
    ...
    lines.push(`## ${idx + 1}. ${eyeIcon} ${event.eyeName || event.eyeId || 'System'} ${stageBadge}${latency}`);
...
export function exportSessionAsPDF(
  ...
  // Footer
  doc.setFontSize(8);
  doc.text(`Exported from Third Eye MCP on ${new Date().toISOString()}`, 20, yPos);
...
export function exportPersonaAsPDF(persona: ExportPersona): Blob {
  ...
  doc.setFontSize(8);
  doc.text(`Exported from Third Eye MCP on ${new Date().toISOString()}`, 20, pageHeight - 10);
...
export function exportSession(
  ...
  switch (format) {
    case 'markdown': {
      ...
      downloadFile(content, `session-${sessionId}-${timestamp}.md`, 'text/markdown');
      break;
    }
    case 'pdf': {
      const blob = exportSessionAsPDF(sessionId, events, metadata);
      downloadFile(blob, `session-${sessionId}-${timestamp}.pdf`, 'application/pdf');
      break;
    }
    case 'json': {
      const content = exportSessionAsJSON(sessionId, events, metadata);
      downloadFile(content, `session-${sessionId}-${timestamp}.json`, 'application/json');
      break;
    }
  }
}
