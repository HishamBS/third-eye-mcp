...
export const MESSAGES = {
  ...
  ERROR_DELETE_SESSIONS_GENERIC: 'Error deleting sessions. Please check console.',
  ...
  ROUTING_SAVED_MULTIPLE: (count: number) => `Saved routing changes for ${count} Eye${count > 1 ? 's' : ''}`,
  ROUTING_DISCARDED: 'Routing changes discarded',
  UNSAVED_CHANGES: (count: number) => `You have unsaved routing changes for ${count} Eye${count > 1 ? 's' : ''}`,
  ...
  CONFIGURE_PROVIDERS_ROUTE_MODELS: 'Configure providers and route models to Eyes',
} as const;
