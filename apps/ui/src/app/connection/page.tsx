          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <h1 className="mt-1 text-2xl font-semibold text-white">Agent Connection Playbook</h1>
              </div>
            </div>
          </div>
        </div>
          ...
          {AGENT_GUIDES.map((guide, index) => {
            ...
            return (
              <motion.section
                ...
                className="rounded-2xl border border-brand-outline/40 bg-brand-paper/80 shadow-glass"
              >
                <button
                  type="button"
                  onClick={() => setOpenId(isOpen ? null : guide.id)}
                  className="flex w-full items-center justify-between gap-3 rounded-2xl px-6 py-4 text-left text-sm text-white transition hover:bg-brand-paperElev/80 focus:outline-none focus:ring-2 focus:ring-brand-accent/50"
                >
                  <div>
                    <h2 className="text-lg font-semibold text-white">{guide.name}</h2>
                    <p className="mt-1 text-xs text-slate-400">{guide.description}</p>
                  </div>
                ...
                {isOpen && (
                  <motion.div
                    ...
                    className="space-y-4 border-t border-brand-outline/30 px-6 py-4 text-xs text-slate-300"
                  >
                    <p>
                      Reference:{' '}
                      <a
                        className="text-brand-accent underline decoration-dotted underline-offset-4 transition hover:text-brand-primary"
                        href={guide.docsUrl}
                        target="_blank"
                        rel="noreferrer"
                      >
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-medium uppercase tracking-[0.2em] text-slate-400">Snippet</label>
                      <div className="relative">
                        <textarea
                          readOnly
                          value={guide.snippet}
                          className="h-36 w-full resize-none rounded-xl border border-brand-outline/40 bg-brand-paper/90 p-3 font-mono text-xs text-green-400 focus:outline-none focus:ring-2 focus:ring-brand-accent/40"
                        />
                        <button
                          type="button"
                          onClick={() => handleCopy(guide.snippet, guide.id)}
                          className="absolute right-3 top-3 rounded-full border border-brand-outline/40 bg-brand-paper/90 px-3 py-1 text-[11px] font-medium text-brand-accent transition hover:border-brand-accent hover:bg-brand-accent/10"
                        >
                    ...
                    {guide.dockerHint && (
                      <p className="rounded-lg border border-yellow-700/40 bg-yellow-900/10 p-3 text-[11px] text-yellow-200">
                        {guide.dockerHint}
                      </p>
                    )}
                  </motion.div>
