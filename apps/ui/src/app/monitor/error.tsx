...
/**
 * Monitor Page Error Boundary
 *
 * Catches errors specific to the Monitor/Theatre page.
 * Common errors: WebSocket failures, session data parsing, timeline rendering.
 */
export default function MonitorError({
  ...
  useEffect(() => {
    console.error("[MonitorError] Monitor page error:", error);
  }, [error]);
    ...
    <div className="min-h-screen flex items-center justify-center bg-brand-surface p-4">
      ...
      >
        ...
        <p className="text-semantic-muted mb-4">
          The session monitor encountered an error. This may be due to:
        </p>
        ...
        <ul className="text-left text-sm text-semantic-muted mb-6 space-y-1">
          ...
          <li>- Timeline rendering failure</li>
        </ul>
