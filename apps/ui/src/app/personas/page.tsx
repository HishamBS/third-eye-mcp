...
function getCleanMission(rawMission: string): string {
  ...
  const firstPart = rawMission.split(/\n\n|GUIDANCE Phase|VALIDATION Phase|Check:|Always respond with|Response \(/i)[0];
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        {persona.active && (
                          <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${STATUS_BG_COLORS_SUBTLE.success} ${STATUS_TEXT_COLORS.success}`}>
                            Active
                          </span>
                        )}
                      </div>
            ...
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                {personaVersions.length > 1 && (
                  <div className="flex items-center gap-2">
                    <label className="text-sm font-medium text-semantic-muted">Version:</label>
                    <select
                  ...
                  <div className="space-y-6">
                    ...
                    <section>
                      <div className="rounded-xl bg-gradient-to-br from-brand-accent/15 to-brand-accent/5 p-6 border-2 border-brand-accent/30 shadow-lg">
                        <h3 className="text-sm font-semibold text-brand-accent uppercase tracking-wide mb-3">Mission</h3>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {data.metadata.capabilities && data.metadata.capabilities.length > 0 ? (
                          data.metadata.capabilities.map((cap) => (
                            <span key={cap} className="inline-flex items-center gap-1.5 rounded-lg bg-brand-accent/20 px-4 py-2 text-sm font-medium text-brand-accent border border-brand-accent/30">
                              <span className="w-1.5 h-1.5 rounded-full bg-brand-accent"></span>
                              {cap}
                            </span>
                          ))
                        ) : (
                          <span className="text-sm text-semantic-muted italic">No capabilities defined</span>
                        )}
                      </div>
                    </section>
                    ...
                    <section>
                      ...
                      <div className="space-y-3">
                        ...
                        {data.guidancePhase && (
                          <div className={`rounded-xl border ${STATUS_BORDER_COLORS_SUBTLE.info} ${STATUS_BG_COLORS_SUBTLE.info} overflow-hidden shadow-sm`}>
                            <button
                            ...
                            {expandedSections.guidance && (
                              <div className="px-4 pb-4 max-h-96 overflow-y-auto space-y-4">
                                {data.guidancePhase.stage && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-blue-400 uppercase tracking-wide mb-1">Stage</h5>
                                    <p className="text-sm text-brand-foreground">{data.guidancePhase.stage}</p>
                                  </div>
                                )}
                                {data.guidancePhase.mission && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-blue-400 uppercase tracking-wide mb-1">Mission</h5>
                                    <div className="prose prose-sm prose-invert max-w-none text-semantic-muted">
                                      <ReactMarkdown remarkPlugins={[remarkGfm]}>{data.guidancePhase.mission}</ReactMarkdown>
                                    </div>
                                  </div>
                                )}
                                {data.guidancePhase.check && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-blue-400 uppercase tracking-wide mb-1">Check</h5>
                                    <div className="prose prose-sm prose-invert max-w-none text-semantic-muted">
                                      <ReactMarkdown remarkPlugins={[remarkGfm]}>{data.guidancePhase.check}</ReactMarkdown>
                                    </div>
                                  </div>
                                )}
                                {data.guidancePhase.reminders && data.guidancePhase.reminders.length > 0 && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-blue-400 uppercase tracking-wide mb-1">Reminders</h5>
                                    <ul className="space-y-1.5">
                                      {data.guidancePhase.reminders.map((reminder, i) => (
                                        <li key={i} className="flex items-start gap-2 text-sm text-semantic-muted">
                                          <span className="text-blue-400 mt-0.5">•</span>
                                          <div className="prose prose-sm prose-invert max-w-none flex-1">
                                            <ReactMarkdown remarkPlugins={[remarkGfm]}>{reminder}</ReactMarkdown>
                                          </div>
                                        </li>
                                      ))}
                                    </ul>
                                ...
                                {data.guidancePhase.example && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-blue-400 uppercase tracking-wide mb-1">Example Response</h5>
                        ...
                        {data.validationPhase && (
                          <div className={`rounded-xl border ${STATUS_BORDER_COLORS_SUBTLE.success} ${STATUS_BG_COLORS_SUBTLE.success} overflow-hidden shadow-sm`}>
                            <button
                            ...
                            {expandedSections.validation && (
                              <div className="px-4 pb-4 max-h-96 overflow-y-auto space-y-4">
                                {data.validationPhase.stage && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-green-400 uppercase tracking-wide mb-1">Stage</h5>
                                    <p className="text-sm text-brand-foreground">{data.validationPhase.stage}</p>
                                  </div>
                                )}
                                {data.validationPhase.mission && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-green-400 uppercase tracking-wide mb-1">Mission</h5>
                                    <div className="prose prose-sm prose-invert max-w-none text-semantic-muted">
                                      <ReactMarkdown remarkPlugins={[remarkGfm]}>{data.validationPhase.mission}</ReactMarkdown>
                                    </div>
                                  </div>
                                )}
                                {data.validationPhase.check && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-green-400 uppercase tracking-wide mb-1">Check</h5>
                                    <div className="prose prose-sm prose-invert max-w-none text-semantic-muted">
                                      <ReactMarkdown remarkPlugins={[remarkGfm]}>{data.validationPhase.check}</ReactMarkdown>
                                    </div>
                                  </div>
                                )}
                                {data.validationPhase.reminders && data.validationPhase.reminders.length > 0 && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-green-400 uppercase tracking-wide mb-1">Reminders</h5>
                                    <ul className="space-y-1.5">
                                      {data.validationPhase.reminders.map((reminder, i) => (
                                        <li key={i} className="flex items-start gap-2 text-sm text-semantic-muted">
                                ...
                                {data.validationPhase.example && (
                                  <div>
                                    <h5 className="text-xs font-semibold text-green-400 uppercase tracking-wide mb-1">Example Response</h5>
                                    <pre className="mt-2 rounded-lg bg-black/30 p-4 text-xs text-semantic-muted overflow-x-auto">
                                      {data.validationPhase.example}
                                    </pre>
                                  </div>
                    ...
                    <section>
                      <div className="rounded-xl border border-brand-outline/50 bg-brand-paper/30 overflow-hidden shadow-sm">
                        ...
                        >
                          <div className="flex items-center gap-2">
                            <span className="text-lg">📦</span>
                            <h3 className="text-base font-semibold text-brand-foreground">Envelope Contract</h3>
                          </div>
                        ...
                        {expandedSections.envelope && (
                          <div className="px-4 pb-4 space-y-4 max-h-96 overflow-y-auto">
                            <div>
                              <p className="mb-2 text-xs font-semibold text-brand-accent uppercase tracking-wide">Required Keys</p>
                              <div className="flex flex-wrap gap-2">
                                {data.envelopeContract.requiredKeys && data.envelopeContract.requiredKeys.length > 0 ? (
                                  data.envelopeContract.requiredKeys.map((key, idx) => (
                                    <span key={idx} className="inline-flex items-center gap-1 rounded-md bg-brand-accent/15 px-3 py-1.5 text-xs font-medium text-brand-accent border border-brand-accent/30">
                                      <span className="text-brand-accent">●</span>
                                      {key}
                                    </span>
                                  ))
                                ) : (
                                  <span className="text-xs text-semantic-muted italic">None</span>
                                )}
                              </div>
                            </div>
                            <div>
                              <p className="mb-2 text-xs font-semibold text-brand-accent uppercase tracking-wide">Required Data Keys</p>
                              <div className="flex flex-wrap gap-2">
                                {data.envelopeContract.requiredDataKeys && data.envelopeContract.requiredDataKeys.length > 0 ? (
                                  data.envelopeContract.requiredDataKeys.map((key, idx) => (
                                    <span key={idx} className="inline-flex items-center gap-1 rounded-md bg-blue-500/15 px-3 py-1.5 text-xs font-medium text-blue-400 border border-blue-500/30">
                            ...
                            <div>
                              <p className="mb-2 text-xs font-semibold text-brand-accent uppercase tracking-wide">Required UI Keys</p>
                              <div className="flex flex-wrap gap-2">
                                {data.envelopeContract.requiredUiKeys && data.envelopeContract.requiredUiKeys.length > 0 ? (
                                  data.envelopeContract.requiredUiKeys.map((key, idx) => (
                                    <span key={idx} className="inline-flex items-center gap-1 rounded-md bg-green-500/15 px-3 py-1.5 text-xs font-medium text-green-400 border border-green-500/30">
                    ...
                    {data.reminders && data.reminders.length > 0 && (
                      <section>
                        <div className="rounded-xl border border-yellow-500/40 bg-yellow-500/10 overflow-hidden shadow-sm">
                          <button
                            onClick={() => setExpandedSections(prev => ({ ...prev, reminders: !prev.reminders }))}
                            className="w-full p-4 flex items-center justify-between text-left hover:bg-yellow-500/20 transition"
                          >
                            <div className="flex items-center gap-2">
                              <span className="text-lg">⚠️</span>
                              <h3 className="text-base font-semibold text-yellow-300">Reminders</h3>
                            </div>
                            <span className="text-yellow-300">{expandedSections.reminders ? '▼' : '▶'}</span>
                          </button>
                          {expandedSections.reminders && (
                            <div className="px-4 pb-4 max-h-80 overflow-y-auto">
                              <ul className="space-y-2">
                                {data.reminders.map((reminder, idx) => (
                                  <li key={idx} className="flex items-start gap-2 text-sm text-yellow-100">
                    ...
                    <section>
                      <div className="rounded-xl border border-purple-500/40 bg-purple-500/10 overflow-hidden shadow-sm">
                        <button
                          onClick={() => setExpandedSections(prev => ({ ...prev, systemPrompt: !prev.systemPrompt }))}
                          className="w-full p-4 flex items-center justify-between text-left hover:bg-purple-500/20 transition"
                        >
                          <div className="flex items-center gap-2">
                            <span className="text-lg">🔧</span>
                            <div>
                              <h3 className="text-base font-semibold text-purple-300">System Prompt (Advanced)</h3>
                              <p className="text-xs text-purple-400/70 mt-0.5">Full technical prompt for developers</p>
                            </div>
                          </div>
                          <span className="text-purple-300">{expandedSections.systemPrompt ? '▼' : '▶'}</span>
                        </button>
                        {expandedSections.systemPrompt && (
                          <div className="px-4 pb-4">
                            <div className="mb-3 rounded-lg bg-purple-900/30 border border-purple-500/30 p-3">
                              <p className="text-xs text-purple-300">
                                ⚠️ <strong>Developer Only:</strong> This is the raw system prompt including technical instructions, JSON schemas, and examples.
                              </p>
                            </div>
                    ...
                    {data.llmConfig && (
                      <section>
                        <div className="rounded-xl border border-brand-outline/50 bg-brand-paper/30 overflow-hidden shadow-sm">
                          <button
                            onClick={() => setExpandedSections(prev => ({ ...prev, llmConfig: !prev.llmConfig }))}
                            className="w-full p-4 flex items-center justify-between text-left hover:bg-brand-paper/50 transition"
                          >
                            <div className="flex items-center gap-2">
                              <span className="text-lg">⚙️</span>
                              <h3 className="text-base font-semibold text-brand-foreground">LLM Configuration</h3>
                            </div>
                            <span className="text-brand-accent">{expandedSections.llmConfig ? '▼' : '▶'}</span>
                          </button>
                          {expandedSections.llmConfig && (
                            <div className="px-4 pb-4 space-y-3">
                              <div className="grid grid-cols-2 gap-3">
                                <div className="rounded-lg bg-brand-paperElev p-3 border border-brand-outline/30">
                                  <p className="text-xs font-semibold text-brand-accent uppercase tracking-wide mb-1">Temperature</p>
                                  <p className="text-sm font-mono text-brand-foreground">{data.llmConfig.temperature ?? 'N/A'}</p>
                                </div>
                                <div className="rounded-lg bg-brand-paperElev p-3 border border-brand-outline/30">
                                  <p className="text-xs font-semibold text-brand-accent uppercase tracking-wide mb-1">Top P</p>
                                  <p className="text-sm font-mono text-brand-foreground">{data.llmConfig.top_p ?? 'N/A'}</p>
                                </div>
                                <div className="rounded-lg bg-brand-paperElev p-3 border border-brand-outline/30">
                                  <p className="text-xs font-semibold text-brand-accent uppercase tracking-wide mb-1">Response Format</p>
                                  <p className="text-sm font-mono text-brand-foreground">{data.llmConfig.response_format ?? 'N/A'}</p>
                                </div>
                                <div className="rounded-lg bg-brand-paperElev p-3 border border-brand-outline/30">
                                  <p className="text-xs font-semibold text-brand-accent uppercase tracking-wide mb-1">Max Tokens</p>
                                  <p className="text-sm font-mono text-brand-foreground">{data.llmConfig.max_tokens ?? 'N/A'}</p>
                                </div>
                              </div>
                            </div>
                          )}
                      ...
                      <section>
                        <div className="rounded-xl border border-brand-outline/50 bg-brand-paper/30 overflow-hidden shadow-sm">
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <span className="text-lg">📝</span>
                              <h3 className="text-base font-semibold text-brand-foreground">Notes</h3>
                            </div>
                            <span className="text-brand-accent">{expandedSections.notes ? '▼' : '▶'}</span>
                          </button>
