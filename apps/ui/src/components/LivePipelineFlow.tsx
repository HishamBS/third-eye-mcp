...
function normalizeApiPipelineEvent(
...
function normalizeWebSocketEvent(message: WSMessage): NormalizedEvent | null {
  ...
  const eye =
    getString(payload.eye) || (result ? getString(result.eye) : undefined);
...
export function LivePipelineFlow({
  ...
  useEffect(() => {
    if (initialEvents.length > 0) {
      const normalized = initialEvents.map((event) =>
        normalizeApiPipelineEvent(event),
      );
  ...
  useEffect(() => {
    if (lastMessage) {
      if (
        lastMessage.type === "pipeline_event" ||
        lastMessage.type === "eye_complete"
      ) {
    ...
    const fetchInitialEvents = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const normalized = payload.map((event: Record<string, unknown>) =>
            normalizeApiPipelineEvent(event),
          );
          normalized.sort((a, b) => a.createdAt - b.createdAt);
          setEvents(normalized);
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute top-2 right-2 z-20">
        ...
        >
          <div
            className={`h-2 w-2 rounded-full ${isConnected ? `${STATUS_TEXT_COLORS.success} animate-pulse` : STATUS_TEXT_COLORS.muted}`}
          />
      ...
      <div className="mt-4 max-h-64 overflow-y-auto rounded-lg bg-brand-paper/50 p-4">
        <h3 className="mb-2 text-sm font-semibold text-semantic-muted">
          Event Feed
        </h3>
              ...
              >
                <div
                  className={`mt-0.5 h-2 w-2 flex-shrink-0 rounded-full ${
                    event.code?.startsWith("OK")
                      ? STATUS_TEXT_COLORS.success
                      : event.code?.startsWith("REJECT")
                        ? STATUS_TEXT_COLORS.error
                        : event.code?.startsWith("NEED")
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-semantic-muted">
                    ...
                    {event.md || event.message || "Event received"}
                  </p>
                </div>
              </div>
            ))}
          {events.length === 0 && (
            <p className="text-center text-semantic-muted">
