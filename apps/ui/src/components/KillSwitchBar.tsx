...
export function KillSwitchBar({
  ...
  const disabled =
    !sessionId || !apiKey || !latestDraft || status === "running";
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const topic = (latestEvent?.data as Record<string, unknown>)?.topic as
        | string
        | undefined;
      ...
      const tenseigan = result?.tenseigan as
        | { ok?: boolean; md?: string }
        | undefined;
      const byakugan = result?.byakugan as
        | { ok?: boolean; md?: string }
        | undefined;
      if (tenseigan?.ok === false || byakugan?.ok === false) {
        ...
        setMessage(
          tenseigan?.md ??
            byakugan?.md ??
            "Validation failed — inspect Eye issues for fixes.",
        );
      } else {
        setStatus("success");
        setMessage(
          "Kill switch completed. Watch the Eyes for updated verdicts.",
        );
      }
    } catch (error) {
      console.error(error);
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Kill switch failed — see Rinnegan instructions.",
      );
    }
  };
  ...
  return (
    <section
      className={clsx(
        ...
        status === "error"
          ? "${STATUS_BORDER_COLORS_SUBTLE.error} ${STATUS_BG_COLORS_SUBTLE.error} ${STATUS_TEXT_COLORS.error}"
          : "border-brand-outline/40 bg-brand-paperElev/70 text-semantic-muted",
      )}
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm">
            Force Tenseigan + Byakugan to re-run on the current draft.
          </p>
        </div>
        <button
          type="button"
