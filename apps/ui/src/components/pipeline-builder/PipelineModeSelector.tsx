...
function ModeCard({
  ...
  const badgeColor =
    badgeColors[info.badgeColor as keyof typeof badgeColors] ||
    badgeColors.blue;
  ...
  return (
    ...
    >
      ...
      {/* Badge */}
      <div
        className={`inline-block px-2 py-0.5 text-xs rounded-full ${badgeColor}`}
      >
        {info.badge}
      </div>
      ...
      <div className="text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
        {currentMode === "fully_dynamic" && (
          <p>
            ...
            automatically selects the optimal eye sequence based on required
            capabilities. Best for most use cases.
          </p>
        )}
        {currentMode === "constrained" && (
          <p>
            <strong>Constrained Mode:</strong> Overseer routes dynamically but
            respects your policy constraints (mandatory eyes, forbidden eyes,
            security requirements). Use when you need guardrails.
          </p>
        )}
        {currentMode === "fixed" && (
          <p>
            <strong>Fixed Template Mode:</strong> Uses a predefined eye sequence
            from a template. Consistent results for repeated tasks. Best for
