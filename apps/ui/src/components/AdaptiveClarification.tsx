...
export function AdaptiveClarification({
  ...
  const isAllAnswered = clarifyingQuestions.every((_, idx) =>
    answers[idx]?.trim(),
  );
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-brand-outline/40 bg-brand-paper/60 p-6">
        ...
        {ambiguousTerms.length > 0 && (
          <div className="mb-4">
            <h4 className="mb-2 text-sm font-medium text-semantic-muted">
        ...
        <div className="rounded-2xl border border-brand-outline/40 bg-brand-paper/60 p-6">
          <div className="mb-6 flex items-center justify-between">
            ...
            <span className="text-sm text-semantic-muted">
              ...
              {clarifyingQuestions.length} answered
            </span>
          </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className="rounded-xl border border-brand-accent/30 bg-brand-accent/5 p-4">
                ...
                <p className="mt-2 text-base text-brand-foreground">
      ...
      {ambiguityScore < 30 && (
        <div
          className={`rounded-xl border ${STATUS_BORDER_COLORS_SUBTLE.success} ${STATUS_BG_COLORS_SUBTLE.success} p-4`}
        >
          <div className="flex items-start gap-3">
            <CheckCircle
              className={`h-5 w-5 ${STATUS_TEXT_COLORS.success} flex-shrink-0 mt-0.5`}
            />
            <div>
              <p className={`font-medium ${STATUS_TEXT_COLORS.success}`}>
                Input is clear!
              </p>
              <p
                className={`mt-1 text-sm ${STATUS_TEXT_COLORS.success} opacity-80`}
              >
