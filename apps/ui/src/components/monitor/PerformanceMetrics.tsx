...
function MetricCard({
  ...
  const getTrendColor = () => {
    switch (trend) {
      case "up":
        return METRIC_COLORS.success;
      case "down":
        return METRIC_COLORS.error;
      default:
        return METRIC_COLORS.muted;
    }
  };
...
export function PerformanceMetrics({
  ...
  const metrics = useMemo(() => {
    ...
    const completedEvents = recentEvents.filter(
      (event) =>
        event.code === "OK" ||
        event.code === "OK_WITH_NOTES" ||
        event.code?.startsWith("REJECT_"),
    );
    const successfulEvents = recentEvents.filter(
      (event) => event.code === "OK" || event.code === "OK_WITH_NOTES",
    );
    const successRate =
    ...
    const avgLatency =
      runLatencies.length > 0
        ? Math.round(
            runLatencies.reduce((sum, lat) => sum + lat, 0) /
              runLatencies.length,
          )
        : 0;

    // Calculate token usage
    const totalTokensIn = runs.reduce(
      (sum, run) => sum + (run.tokensIn || 0),
      0,
    );
    const totalTokensOut = runs.reduce(
    ...
    const recentHourEvents = events.filter(
      (event) => new Date(event.createdAt).getTime() > hourAgo,
    );
    const eventsPerHour = recentHourEvents.length;

    // Calculate unique eyes used
    const uniqueEyes = new Set(events.filter((e) => e.eye).map((e) => e.eye))
      .size;
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              ...
              style={{
                color:
                  metrics.successRate >= 80
                    ? METRIC_COLORS.success
                    : metrics.successRate >= 60
                      ? METRIC_COLORS.warning
                      : METRIC_COLORS.error,
              }}
            />
          }
        ...
        <MetricCard
          ...
          icon={
            <BarChart3
              className="h-5 w-5"
              style={{ color: METRIC_COLORS.info }}
            />
          }
          color={METRIC_COLORS.info}
          trend="neutral"
        ...
        <MetricCard
          ...
          unit="eyes"
          icon={
            <Eye className="h-5 w-5" style={{ color: METRIC_COLORS.primary }} />
          }
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <MetricCard
          ...
          icon={
            <ArrowDownToLine
              className="h-5 w-5"
              style={{ color: METRIC_COLORS.accent }}
            />
          }
        ...
        <div className="mt-3 flex space-x-1">
          ...
          {Array.from({ length: 24 }, (_, i) => {
            ...
            const height = Math.max(
              4,
              Math.min(32, (hourEvents.length / 10) * 32),
            );
