...
export function DuelLauncher({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (selectedConfigs.length < 2) {
      setSelectionError(
        "Select at least two model configurations to launch a duel.",
      );
      return;
    }
    try {
      ...
      setStatus("error");
      setMessage(
        error instanceof Error ? error.message : "Failed to launch duel.",
      );
    }
  };
  ...
  const launchDisabled =
    submitting || !prompt.trim() || selectedConfigs.length < 2;
  ...
  return (
    <section className="rounded-2xl border border-brand-outline/40 bg-brand-paperElev/70 p-5 text-sm text-semantic-muted">
      ...
      <p className="mt-2 text-sm text-semantic-muted">
        Compare multiple LLM providers/models side-by-side with identical
        prompts.
      </p>
      <form onSubmit={handleSubmit} className="mt-3 space-y-4">
        ...
        <fieldset className="space-y-3 rounded-xl border border-brand-outline/40 bg-brand-paper/70 p-3">
          <legend className="px-2 text-xs uppercase tracking-[0.2em] text-brand-accent">
          ...
          {roster.length ? (
            <div className="grid gap-2 sm:grid-cols-2">
              {roster.map((config) => {
                ...
                const checked = selectedConfigs.some(
                  (c) =>
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <span className="font-mono text-xs text-semantic-muted">
                      {label}
                    </span>
                  </label>
                );
            ...
            <p className="text-xs text-semantic-muted">
              No configurations available. Add custom configurations below.
            </p>
          )}
          <p className="text-xs text-semantic-muted">
            Select between 2 and 4 model configurations.
          </p>
        </fieldset>
        ...
        <button
          ...
          className={clsx(
            ...
            submitting
              ? "bg-brand-accent/40 text-brand-foreground"
              : launchDisabled
                ? "bg-brand-accent/40 text-brand-foreground opacity-70"
