...
function tabContent(tabId: string, state: EyeState | null) {
  ...
  switch (tabId) {
    ...
    case "why":
      return typeof data.reasoning_md === "string"
        ? data.reasoning_md
        : "Reasoning not provided.";
    case "issues":
      return typeof data.issues_md === "string"
        ? data.issues_md
        : "No issues logged.";
    case "fixes":
            ...
            <section className="mt-6 max-h-[24rem] overflow-y-auto rounded-xl border border-brand-outline/40 bg-brand-paper p-5 text-sm text-brand-foreground">
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: renderMarkdown(tabContent(activeTab, state)),
                }}
              />
            </section>
          </motion.div>
