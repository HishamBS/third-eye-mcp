...
export function ReviewStep({ state, onSave }: ReviewStepProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(error instanceof Error ? error.message : VALIDATION_MESSAGES.GENERIC_ERROR);
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-xl font-bold text-brand-foreground">{REVIEW_LABELS.TITLE}</h2>
        <p className="mt-1 text-sm text-semantic-muted">{REVIEW_LABELS.SUBTITLE}</p>
      </div>

      {/* Validation Status */}
      {validationErrors.length > 0 ? (
        <div className={`rounded-lg border ${STATUS_BORDER_COLORS_SUBTLE.error} ${STATUS_BG_COLORS_SUBTLE.error} p-4`}>
          <p className={`mb-2 text-sm font-semibold ${STATUS_TEXT_COLORS.error}`}>
            {REVIEW_LABELS.VALIDATION_ERROR}
          </p>
          <ul className={`list-inside list-disc space-y-1 text-sm ${STATUS_TEXT_COLORS.error}`}>
        ...
        <div className={`rounded-lg border ${STATUS_BORDER_COLORS_SUBTLE.success} ${STATUS_BG_COLORS_SUBTLE.success} p-4`}>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-brand-outline/50 bg-brand-paperElev p-4">
          <h3 className="mb-3 text-sm font-semibold text-brand-accent">{REVIEW_LABELS.SECTION_BASIC_INFO}</h3>
          <div className="space-y-2 text-sm">
            <div>
              <span className="font-medium text-semantic-muted">Name:</span>
              <span className="ml-2 text-brand-foreground">{state.formData.name || 'N/A'}</span>
            </div>
            <div>
              <span className="font-medium text-semantic-muted">Description:</span>
              <p className="mt-1 text-brand-foreground">{state.formData.description || 'N/A'}</p>
            </div>
            {state.formData.iconSvg && (
              <div>
                <span className="font-medium text-semantic-muted">Custom Icon:</span>
        ...
        {/* Schemas */}
        <div className="rounded-lg border border-brand-outline/50 bg-brand-paperElev p-4">
          <h3 className="mb-3 text-sm font-semibold text-brand-accent">{REVIEW_LABELS.SECTION_SCHEMAS}</h3>
          <div className="space-y-2 text-sm">
            <div>
              <span className="font-medium text-semantic-muted">Input Schema:</span>
              <pre className="mt-1 max-h-32 overflow-auto rounded bg-brand-ink p-2 text-xs text-status-success">
                {state.formData.inputSchema}
              </pre>
            </div>
            <div>
              <span className="font-medium text-semantic-muted">Output Schema:</span>
              <pre className="mt-1 max-h-32 overflow-auto rounded bg-brand-ink p-2 text-xs text-status-success">
        ...
        {/* Persona */}
        <div className="rounded-lg border border-brand-outline/50 bg-brand-paperElev p-4">
          <h3 className="mb-3 text-sm font-semibold text-brand-accent">{REVIEW_LABELS.SECTION_PERSONA}</h3>
          <div className="text-sm">
            {state.formData.personaId ? (
              <span className="text-brand-foreground">{state.formData.personaId}</span>
            ) : (
