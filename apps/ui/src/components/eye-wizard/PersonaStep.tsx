...
export function PersonaStep({ state, dispatch }: WizardStepProps) {
      ...
      <div className="flex min-h-[400px] items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-semantic-muted">
            {PERSONA_LABELS.PERSONA_LOADING}
          </p>
        </div>
      </div>
    );
  ...
  if (error) {
    return (
      <div className="flex min-h-[400px] items-center justify-center">
        <div
          className={`rounded-lg border ${STATUS_BORDER_COLORS_SUBTLE.error} ${STATUS_BG_COLORS_SUBTLE.error} p-6 text-center`}
        >
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-semantic-muted">
          {PERSONA_LABELS.PERSONA_SELECT_HELP}
        </p>
      </div>
      ...
      {selectedPersona && (
        <div className="rounded-lg border border-brand-outline/50 bg-brand-paperElev p-6">
          <div className="mb-4 flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-semantic-muted">
                Version {selectedPersona.version}
              </p>
            </div>
            {selectedPersona.active && (
              <span
                className={`rounded-full ${STATUS_BG_COLORS_SUBTLE.success} px-3 py-1 text-xs font-medium ${STATUS_TEXT_COLORS.success}`}
              >
                Active
              </span>
            )}
          ...
          {selectedPersona.mission && (
            <div className="mb-4">
              <h4 className="mb-2 text-sm font-semibold text-brand-foreground">
                Mission
              </h4>
              <p className="text-sm text-semantic-muted">
                {selectedPersona.mission}
          ...
          {/* Capabilities */}
          {selectedPersona.metadata?.capabilities &&
            selectedPersona.metadata.capabilities.length > 0 && (
              <div>
                <h4 className="mb-2 text-sm font-semibold text-brand-foreground">
                  Capabilities
                </h4>
      ...
      {!selectedPersona && (
        ...
        >
          <p className={`text-sm ${STATUS_TEXT_COLORS.info}`}>
            💡 <strong>Optional:</strong> Linking a Persona to your Eye enhances
            its capabilities with specialized behavior, guidance phases, and
            validation rules. You can skip this step and add a Persona later.
          </p>
        </div>
      )}
    </div>
