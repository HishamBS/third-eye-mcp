 ...
 */
function wizardReducer(state: EyeFormState, action: EyeFormAction): EyeFormState {
  switch (action.type) {
    case 'SET_STEP':
    ...
    case 'NEXT_STEP':
      if (state.currentStep < WIZARD_CONFIG.LAST_STEP) {
        return { ...state, currentStep: (state.currentStep + 1) as EyeWizardStep };
      }
      return state;
...
export function EyeWizard({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="mb-6">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-sm font-medium text-semantic-muted">
            {PROGRESS_LABELS.STEP_OF(state.currentStep, WIZARD_CONFIG.TOTAL_STEPS)}
          </span>
          <span className="text-sm font-medium text-brand-accent">
            {PROGRESS_LABELS.PROGRESS_PERCENTAGE(state.currentStep, WIZARD_CONFIG.TOTAL_STEPS)}
          </span>
        </div>
      ...
      {/* Current Step Header */}
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-brand-foreground">{currentStepConfig.title}</h2>
        <p className="mt-1 text-sm text-semantic-muted">{currentStepConfig.description}</p>
      </div>
