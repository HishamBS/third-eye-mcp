...
export function SecurityBanner() {
  ...
  return (
    <div
      className={`fixed top-0 left-0 right-0 z-50 ${STATUS_BG_COLORS_SUBTLE.error} text-brand-foreground shadow-lg`}
    >
      <div className="max-w-7xl mx-auto px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between flex-wrap">
          <div className="flex items-center flex-1">
            ...
            <p className="font-medium text-sm sm:text-base">
              <span className="font-bold">Security Warning:</span> Server is
              exposed on{" "}
              ...
              <span className="hidden sm:inline">
                {" "}
                This is a local-first tool. For security, bind to 127.0.0.1
                only.
              </span>
            </p>
          </div>
        ...
        <div className={`mt-2 text-xs sm:text-sm ${STATUS_TEXT_COLORS.error}`}>
          To fix: Set{" "}
          <code className="bg-brand-ink px-1 py-0.5 rounded font-mono">
            HOST=127.0.0.1
          </code>{" "}
          in environment or restart without network binding.
        </div>
      </div>
    </div>
  );
