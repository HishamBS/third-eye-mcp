...
export function SessionHeader({
  ...
  const [draftSettings, setDraftSettings] = useState<SessionSettingsPayload>(
    () => settings ?? {},
  );
  ...
  const sessionMeta = useMemo(() => {
    return (
      sortedSessions.find((item) => item.session_id === draftSession) ?? null
    );
    ...
    <section className="rounded-2xl border border-brand-outline/60 bg-brand-paperElev/80 px-6 py-6 text-sm text-semantic-muted shadow-glass md:px-8 md:py-7">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-semantic-muted">
            Connect to a live Overseer session to stream Eye envelopes in real
            time. API keys are never stored remotely; they live in your browser
            storage only.
          </p>
        </div>
      ...
      >
        <div className="flex flex-col gap-2">
          <label
            className="text-xs uppercase tracking-[0.2em] text-semantic-muted"
          ...
          <div className="flex flex-col gap-2">
            ...
            {sessionsError && (
              <p className="text-xs ${STATUS_TEXT_COLORS.error}">
                {sessionsError}
              </p>
            )}
            {sessionMeta && !showSessionSkeleton && (
              <dl className="grid gap-3 rounded-xl border border-brand-outline/40 bg-brand-paper/70 px-3 py-2 text-xs text-semantic-muted sm:grid-cols-3">
                <div>
                  <dt className="uppercase tracking-[0.2em] text-semantic-muted">
                    Status
                  </dt>
                  <dd>
                    <span
                      className={clsx(
                        ...
                        sessionStatusTone(sessionMeta.status),
                      )}
                    >
                ...
                <div>
                  ...
                  <dd className="mt-1 text-semantic-muted">
                    {sessionMeta.tenant ?? "—"}
                  </dd>
                </div>
                <div>
                  ...
                  <dd className="mt-1 text-semantic-muted">
                    {formatTimestamp(
                      sessionMeta.last_event_at ?? sessionMeta.created_at,
                    )}
                  </dd>
                </div>
              </dl>
        ...
        <label className="flex flex-col gap-2">
          <span className="text-xs uppercase tracking-[0.2em] text-semantic-muted">
            API Key
          </span>
          <input
            value={apiKey}
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        ...
        <form
          className="rounded-xl border border-brand-outline/40 bg-brand-paper/80 p-4"
        ...
        >
          ...
          <p className="mt-1 text-xs text-semantic-muted">
            Tune Overseer strictness per session.
          </p>
          ...
          <label className="mt-3 flex flex-col gap-2">
            <span>
              Ambiguity threshold:{" "}
              {Math.round((draftSettings.ambiguity_threshold ?? 0.35) * 100)}%
            </span>
            <input
              type="range"
              min="0"
              ...
              onChange={(event) =>
                setDraftSettings((prev) => ({
                  ...prev,
                  ambiguity_threshold: Number(event.target.value),
                }))
              }
            />
          </label>
          ...
          <label className="mt-3 flex flex-col gap-2">
            <span>
              Citation cutoff:{" "}
              {Math.round((draftSettings.citation_cutoff ?? 0.8) * 100)}%
            </span>
            <input
              type="range"
              min="0"
              ...
              onChange={(event) =>
                setDraftSettings((prev) => ({
                  ...prev,
                  citation_cutoff: Number(event.target.value),
                }))
              }
            />
          </label>
          ...
          <label className="mt-3 flex items-center justify-between gap-2">
            ...
            <input
              ...
              onChange={(event) =>
                setDraftSettings((prev) => ({
                  ...prev,
                  require_rollback: event.target.checked,
                }))
              }
              className="h-4 w-8 cursor-pointer rounded-full border border-brand-outline/50 bg-brand-paper accent-brand-accent"
            />
          ...
          <label className="mt-3 flex flex-col gap-2">
            <span>
              ...
              {Math.round((draftSettings.consistency_tolerance ?? 0.85) * 100)}%
            </span>
            <input
              type="range"
              min="0"
              ...
              onChange={(event) =>
                setDraftSettings((prev) => ({
                  ...prev,
                  consistency_tolerance: Number(event.target.value),
                }))
              }
            />
          </label>
          ...
          {feedback && (
            <p className="mt-2 text-xs text-semantic-muted">{feedback}</p>
          )}
        </form>
        ...
        <div className="rounded-xl border border-brand-outline/40 bg-brand-paper/80 p-4">
          ...
          <ul className="mt-2 space-y-2 text-xs text-semantic-muted">
            ...
            <li>
              🟡 Amber chip — Awaiting response or pending clarifications.
            </li>
          </ul>
        </div>
      </div>
