...
export async function submitClarifications(options: {
  ...
  const response = await fetch(
    `${base}/api/session/${encodeURIComponent(sessionId)}/clarifications`,
    {
      method: "POST",
      headers: buildHeaders(apiKey),
...
export async function updateSessionSettings(options: {
  ...
  const base = buildBaseUrl();
  const response = await fetch(
    `${base}/api/session/${encodeURIComponent(sessionId)}/settings`,
    {
      method: "PUT",
      headers: buildHeaders(apiKey),
...
export async function postKillSwitch(options: {
  ...
  const base = buildBaseUrl();
  const response = await fetch(
    `${base}/api/session/${encodeURIComponent(sessionId)}/revalidate`,
    {
      method: "POST",
      headers: buildHeaders(apiKey),
...
export function buildWebSocketUrl(sessionId: string, apiKey?: string): string {
  ...
  const url = new URL(
    `${wsBase}/ws/monitor?sessionId=${encodeURIComponent(sessionId)}`,
  );
  if (apiKey) {
...
export async function postResubmitRequest(options: {
  ...
  const base = buildBaseUrl();
  const response = await fetch(
    `${base}/api/session/${encodeURIComponent(sessionId)}/resubmit`,
    {
      method: "POST",
      headers: buildHeaders(apiKey),
...
export async function fetchLeaderboard(options: {
  ...
  const base = buildBaseUrl();
  const response = await fetch(
    `${base}/api/session/${encodeURIComponent(sessionId)}/leaderboard`,
    {
      method: "GET",
      headers: buildHeaders(apiKey),
    },
...
export async function fetchSessionSummary(options: {
  ...
  const base = buildBaseUrl();
  const response = await fetch(
    `${base}/api/session/${encodeURIComponent(sessionId)}/summary`,
    {
      method: "GET",
      headers: buildHeaders(apiKey),
      signal,
