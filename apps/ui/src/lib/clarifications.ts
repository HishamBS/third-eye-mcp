...
function normaliseQuestionLine(
  line: string,
  index: number,
): ClarificationQuestion {
...
export function extractClarifications(
  ...
  const data = eye.data ?? {};
  const questionsMd =
    typeof data.questions_md === "string" ? data.questions_md : "";
  const score = typeof data.score === "number" ? data.score : undefined;
  const isCodeRelated =
    typeof data.is_code_related === "boolean"
      ? data.is_code_related
      : undefined;
