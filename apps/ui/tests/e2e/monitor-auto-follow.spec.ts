  ...
  eyes: [
    {
      ...
      code: "OK",
      tool_version: "1.2.3",
      last_event_at: "2025-01-01T10:04:00Z",
    },
  ],
};
...
test.describe("Truth Monitor auto-follow", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/session/**/events?limit=*", async (route) => {
      await route.fulfill({
        status: 200,
        headers,
        body: JSON.stringify(emptyEvents),
      });
    });
  ...
  test("populates roster, shows hero metrics, and defaults auto-follow on", async ({
    page,
  }) => {
    ...
    await expect(
      sessionDropdown.locator('option[value="session-1"]'),
    ).toHaveText(/Session One/);
    await expect(
      sessionDropdown.locator('option[value="session-2"]'),
    ).toHaveText(/Session Two/);
