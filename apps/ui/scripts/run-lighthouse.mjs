...
const BASE_URL =
  process.env.LIGHTHOUSE_URL ?? "http://127.0.0.1:4173/session/session-1";
...
async function run() {
  ...
  try {
    ...
    if (error.code === "ENOENT") {
      console.error(
        "Unable to locate the `lighthouse` binary. Run `bun install` to ensure devDependencies are installed.",
      );
    } else {
