...
describe("Session Endpoints Integration Tests", () => {
  ...
  describe("Context Management", () => {
    ...
    it("should remove context item", async () => {
      const res = await app.request(
        `/sessions/${testSessionId}/context/projectName`,
        {
          method: "DELETE",
        },
      );
  ...
  describe("Session Export", () => {
    it("should export session as JSON", async () => {
      const res = await app.request(
        `/sessions/${testSessionId}/export?format=json`,
      );
      expect(res.status).toBe(200);
    ...
    it("should export session as Markdown", async () => {
      const res = await app.request(
        `/sessions/${testSessionId}/export?format=md`,
      );
      expect(res.status).toBe(200);
    ...
    it("should export session as CSV", async () => {
      const res = await app.request(
        `/sessions/${testSessionId}/export?format=csv`,
      );
      expect(res.status).toBe(200);
    ...
    it("should reject invalid export format", async () => {
      const res = await app.request(
        `/sessions/${testSessionId}/export?format=invalid`,
      );
      expect(res.status).toBe(400);
    });
  ...
  describe("Pagination", () => {
    it("should paginate session runs", async () => {
      const res = await app.request(
        `/sessions/${testSessionId}/runs?limit=10&offset=0`,
      );
      expect(res.status).toBe(200);
    ...
    it("should paginate session events", async () => {
      const res = await app.request(
        `/sessions/${testSessionId}/events?limit=50&offset=0`,
      );
      expect(res.status).toBe(200);
