...
app.get("/", async (c) => {
  try {
    ...
    const profiles = await db
      ...
      .orderBy(
        desc(strictnessProfiles.isBuiltIn),
        desc(strictnessProfiles.createdAt),
      )
      .all();
    ...
    return createInternalErrorResponse(
      c,
      `Failed to fetch strictness profiles: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
});
...
app.post(
  "/",
  validateBodyWithEnvelope(createStrictnessProfileSchema),
  async (c) => {
    try {
      const {
        name,
...
app.put(
  "/:id",
  validateBodyWithEnvelope(updateStrictnessProfileSchema),
  async (c) => {
    try {
      const id = c.req.param("id");
      ...
      await db
        ...
        .set({
          ...
          citationCutoff:
            body.citationCutoff !== undefined
              ? body.citationCutoff
              : existing[0].citationCutoff,
          consistencyTolerance:
