...
app.post('/run', validateBodyWithEnvelope(schemas.mcpRun), async (c) => {
  try {
    const { eye: explicitEye, input, task, sessionId } = c.get('validatedBody');
    ...
    if (task && !explicitEye) {
      ...
      return createErrorResponse(c, {
        ...
        status: 400,
        detail: 'Either provide { eye, input } for explicit routing or { task } for auto-routing',
      });
    }
