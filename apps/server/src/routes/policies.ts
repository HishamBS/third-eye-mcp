...
app.get('/', async (c) => {
  try {
    ...
    const filters = isActiveParam !== undefined ? { isActive: isActiveParam === 'true' } : undefined;
...
app.post('/', async (c) => {
  try {
    ...
    if (!body.name || !body.mandatoryEyes || !Array.isArray(body.mandatoryEyes)) {
      return createErrorResponse(c, 'name and mandatoryEyes (array) are required', 400);
    }
...
app.post('/:id/activate', async (c) => {
  try {
    ...
    return createSuccessResponse(c, { message: 'Policy activated successfully' });
  } catch (error) {
...
app.post('/:id/deactivate', async (c) => {
  try {
    ...
    return createSuccessResponse(c, { message: 'Policy deactivated successfully' });
  } catch (error) {
