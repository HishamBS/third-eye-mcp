...
describe("Phase 4: WebSocket Fine-Grained Events", () => {
  ...
  beforeAll(async () => {
    ...
    vi.spyOn(wsManager, "broadcastToSession").mockImplementation(
      (sessionId: string, event: WebSocketEvent) => {
  ...
  it("should emit eye_started event when Eye begins execution", async () => {
    ...
    const testInput =
      "Generate a comprehensive palm care guide for beginners in Saudi Arabia";
  ...
  it("should persist all events to pipeline_events table", async () => {
    ...
    const parsedEvent =
      typeof firstEvent.dataJson === "string"
  ...
  it("should include UI metadata in Eye responses", async () => {
    ...
    const completeEvent = capturedEvents.find(
      (e) => e.type === "eye_complete" && e.result,
    );
  ...
  it("should broadcast events only to specific session", async () => {
    ...
    const allForTestSession = capturedEvents.every(
      (e) => e.sessionId === testSessionId,
    );
    expect(allForTestSession).toBe(true);

    // Should have no events for session2
    const eventsForSession2 = capturedEvents.filter(
      (e) => e.sessionId === session2.sessionId,
    );
